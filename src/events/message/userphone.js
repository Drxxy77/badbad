module.exports = {
    async execute(Bot, message) {
        ...
        if (keys.includes(message.channel.id)) {
            ...
            if (receiver.toLowerCase() == "searching" || receiver.toLowerCase() == "dead") return;
            message.client.channels.cache.get(receiver).send(`${message.author.tag} 📞 ${RemoveMention(message.content)}`)
        }
        ...
        for (key of keys) {
            if (calls[key] == message.channel.id) {
                ...
                if (host.toLowerCase() == "searching" || host.toLowerCase() == "dead") return;
                message.client.channels.cache.get(host).send(`${message.author.tag} 📞 ${RemoveMention(message.content)}`)
            }
        }
    }
}
